...
model_seeds <- c(10209,102191,53813,19301,74923,86029,87613,87589,77419,44971,74093,
...
for (prop in c('prop04','prop02')){
  for (split_seed in 1:10){
    ...
    if (prop == 'prop02'){
      n_02 <- append(n_02, sum(rowMeans(all_probas) > 0.025))
    } else {
      n_04 <- append(n_04, sum(rowMeans(all_probas) > 0.025))
    }
    
    
    if (prop == 'prop02'){
      gains_02 <- append(gains_02, ganancia_total(test_data[ , clase01 ] , rowMeans(all_probas) ) )
    } else {
      gains_04 <- append(gains_04, ganancia_total(test_data[ , clase01 ] , rowMeans(all_probas) ) )
    }
