...
fwrite( prediccion_final, 
        file= "entrega/rubinstein-mayo_recuperatorio_probas.txt", 
        sep= "\t", 
        eol= "\r\n")
...
fwrite( top_n_predictions[,'numero_de_cliente'], 
        file= "entrega/rubinstein-mayo_recuperatorio_entregar.txt",
        col.names=FALSE, 
        sep= "\t", 
        eol= "\r\n")

writeLines(as.character(threshold), "entrega/rubinstein-mayo_threshold_entrega.txt")

#grabo la importancia de las variables
write.table(  xgb.importance( model = modelo )
              , file= "entrega/rubinstein-mayo_recuperatorio_importancia.txt")
              , sep= "\t"
              , eol= "\r\n"
              , row.names= FALSE
)
